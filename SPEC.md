...
Each post also has a short, shareable URL on the alias domain `cmpst.org` —
`https://cmpst.org/p/[token]`, where `[token]` is a Sqids-encoded form of the
post id. Cloudflare rewrites the host at the edge to
`https://compostmodernism.org/p/[token]`; the app resolves the token and
301s to the post's *current* canonical permalink (so renames don't break old
short links). The `shortlink_redirects` table is the migration-time backstop
for any future change to the Sqids encoder config — see NOTES.md.
